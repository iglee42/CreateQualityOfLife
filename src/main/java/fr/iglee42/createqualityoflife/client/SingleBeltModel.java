...
public class SingleBeltModel extends BakedModelWrapper<BakedModel> {
    public static final ModelProperty<SingleBeltBlockEntity.CasingType> CASING_PROPERTY = new ModelProperty<>();
    ...
    public SingleBeltModel(BakedModel template) {
        super(template);
    }
    @Override
    public TextureAtlasSprite getParticleIcon(IModelData data) {
        ...
        SingleBeltBlockEntity.CasingType type = data.getData(CASING_PROPERTY);
        if (type == SingleBeltBlockEntity.CasingType.NONE || type == SingleBeltBlockEntity.CasingType.BRASS)
            return super.getParticleIcon(data);
    ...
    public List<BakedQuad> getQuads(BlockState state, Direction side, Random rand, IModelData extraData) {
        ...
        SingleBeltBlockEntity.CasingType type = extraData.getData(CASING_PROPERTY);
        boolean brassCasing = type == SingleBeltBlockEntity.CasingType.BRASS;

        if (type == SingleBeltBlockEntity.CasingType.NONE || brassCasing && !cover)
            return quads;

        quads = new ArrayList<>(quads);

        if (cover) {
            boolean alongX = state.getValue(SingleBeltBlock.HORIZONTAL_FACING)
                    .getAxis() == Axis.X;
