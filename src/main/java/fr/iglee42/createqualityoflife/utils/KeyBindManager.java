...
public class KeyBindManager {
    ...
    public static void onClientTick(ClientTickEvent.Post evt) {
        ...
        if (HELMET_EFFECT_KEY.consumeClick()){
            PacketDistributor.sendToServer(new ToggleArmorEffectPacket(EquipmentSlot.HEAD));
        }
        if (CHESTPLATE_EFFECT_KEY.consumeClick()){
            PacketDistributor.sendToServer(new ToggleArmorEffectPacket(EquipmentSlot.CHEST));
        }
        if (LEGGINGS_EFFECT_KEY.consumeClick()){
            PacketDistributor.sendToServer(new ToggleArmorEffectPacket(EquipmentSlot.LEGS));
        }
        if (BOOTS_EFFECT_KEY.consumeClick()){
            PacketDistributor.sendToServer(new ToggleArmorEffectPacket(EquipmentSlot.FEET));
        }
