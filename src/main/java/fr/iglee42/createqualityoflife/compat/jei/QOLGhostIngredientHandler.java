...
public class QOLGhostIngredientHandler<T extends GhostItemMenu<?>>
		...
		if (ingredient.getType() == VanillaTypes.ITEM_STACK) {
			for (int i = 36; i < gui.getMenu().slots.size(); i++) {
				if (gui.getMenu().slots.get(i)
					.isActive() && gui.getMenu().slots.get(i).mayPlace(ingredient.getItemStack().orElse(new ItemStack(Items.AIR))))
	...
	private static class GhostTarget<I, T extends GhostItemMenu<?>> implements Target<I> {
		...
		public void accept(I ingredient) {
			...
			stack.setCount(1);
			if (!gui.getMenu().ghostInventory.isItemValid(slotIndex,stack)) return;
